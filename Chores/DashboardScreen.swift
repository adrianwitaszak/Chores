                ...
                ScrollView {
                    VStack(spacing: 15) {
                        ForEach(viewModel.todosFiltered, id: \.id) { todo in
                            if searchText == "" {
                                TodoView(todo: Todo(ownerId: todo.ownerId, title: todo.title, description: todo.description, isCompleted: todo.isCompleted, documentID: todo.documentID), viewModel: viewModel)
                            } else {
